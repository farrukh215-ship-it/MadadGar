...
export async function GET(request: NextRequest) {
  ...
  const body: Record<string, number | string | null> = { p_limit: limit };
  if (categoryId && categoryId !== 'null') body.p_category_id = categoryId;
  if (city && city !== 'null') body.p_city = city;

  const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/feed_top_rated`, {
    method: 'POST',
    headers: {
  ...
  if (!res.ok) {
    ...
    console.error('[feed/top-rated]', res.status, err);
