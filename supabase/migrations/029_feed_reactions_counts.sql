-- Update feed_nearby to include like_count, dislike_count, reviews_count, comments_count
-- Must DROP first because return type changes (new columns)
...
RETURNS TABLE (
  ...
  SELECT
    ...
    (SELECT COUNT(*) FROM public.post_reactions pr WHERE pr.post_id = p.id AND pr.reaction = 'like') as like_count,
    (SELECT COUNT(*) FROM public.post_reactions pr WHERE pr.post_id = p.id AND pr.reaction = 'dislike') as dislike_count,
    (SELECT COUNT(*) FROM public.ratings r WHERE r.post_id = p.id AND r.review_text IS NOT NULL AND r.review_text != '') as reviews_count,
...
-- Update feed_top_rated similarly
